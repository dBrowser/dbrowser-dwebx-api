...
const {NotFoundError, EntryAlreadyExistsError, DestDirectoryNotEmpty, ArchiveNotWritableError} = require('dbrowser-error-constants')
...
function toBeakerError (err, info) {
  if (err.code == 16) {
    ...
    console.error(`Dbrowser-Dwebx-API: Unhandled error type from ${info}`, err)
